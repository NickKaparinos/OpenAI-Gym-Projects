...
if __name__ == '__main__':
    ...
    if use_prioritised_replay_buffer:
        ...
        train_collector = ts.data.Collector(policy, train_envs,
                                            ts.data.VectorReplayBuffer(total_size=prioritized_buffer_hyperparameters['total_size']*num_envs, buffer_num=num_envs),
                                            exploration_noise=True)
    ...
    # Training
    trainer_hyperparameters = {'max_epoch': 4, 'step_per_epoch': 250_000, 'step_per_collect': 10,
    ...
    result = ts.trainer.offpolicy_trainer(policy, train_collector, test_collector, **trainer_hyperparameters,
                                          train_fn=build_sigma_schedule(**build_sigma_hyperparameters,
                                                                        steps_per_epoch=trainer_hyperparameters[
                                                                            'step_per_epoch']),
                                          test_fn=build_test_fn(num_episodes=8), stop_fn=None,
                                          logger=logger)
    ...
    # Execution Time
    end = time.perf_counter()  # tensorboard --logdir './MuJoCo/Hopper/logs'
