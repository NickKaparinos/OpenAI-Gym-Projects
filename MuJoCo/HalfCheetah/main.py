...
if __name__ == '__main__':
    ...
    train_envs = ts.env.DummyVectorEnv([lambda: gym.make(env_id) for _ in range(num_envs)])
    test_envs = ts.env.DummyVectorEnv([lambda: gym.make(env_id) for _ in range(num_envs)])
    ...
    # Training
    trainer_hyperparameters = {'max_epoch': 4, 'step_per_epoch': 100_000, 'step_per_collect': 100,
                               'episode_per_test': 10, 'batch_size': 64}
    decay_steps = int(trainer_hyperparameters['max_epoch'] * trainer_hyperparameters['step_per_epoch'] * 0.3)
    build_sigma_hyperparameters = {'max_sigma': 0.5, 'min_sigma': 0.1, 'decay_time_steps': decay_steps}
    ...
    # Execution Time
    end = time.perf_counter()  # tensorboard --logdir './MuJoCo/HalfCheetah/logs'
